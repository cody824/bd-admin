sureUploader = {};
$.cachedScript("/js/uploader/ImgProgress.js");
$.cachedScript("/js/imgwrap.js");
$.cachedScript("/js/lib/plupload/plupload.dev.js");
...
function SIUploader(op, callback, checkImgSize, img, pb) {
	...
	var options = $.extend({}, me.defaults, op, {
		init : {
			'FilesAdded' : function(up, files) {
				...
				plupload.each(files, function(file) {
					...
					if(!(file.type=="image/png"||file.type=="image/jpeg"||file.type=="image/bmp")){
						SureMsg.msg("只支持非gif格式的图片文件");
						if(setAllClickEvent()!=undefined)
							setAllClickEvent();
						return;
					}
			...
			'UploadProgress' : function(up, file) {
				 var progressbar = file.loadingBar;
	...
	this.newLoadingBar = function(){
		me.currentFile.loadingBar = new ImgProgress(me.qiniuUploader, me.currentFile, me.target, me.showImg, me.progressBar, callback);
	};
...
function getObjectURL(file) {
	...
	try{
		if (window.createObjectURL != undefined) {// basic
			...
			/*var files=document.all.file.item(0);
			files.select();
			files.blur();
			url=document.selection.createRange().text;*/
			url="/img/icons/Desert.jpg";
		}
	} catch(e){
		//console.log(e);
	}
